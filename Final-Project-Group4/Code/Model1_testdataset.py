...
METRICS_DIR = os.getcwd() + '/model1metrics/'
IMAGE_SIZE = 224
...
def test():
    with torch.no_grad():
        ...
        pd.DataFrame(test_loss_item, columns = ['batch','loss','acc']).to_csv(METRICS_DIR+'test_loss_hist_item1.csv', index =False)
        pd.DataFrame(pred_labels_per_hist, columns = test_labels).to_csv(METRICS_DIR+'pred_labels_per1.csv', index = False)
        pd.DataFrame(pred_labels_hist, columns = test_labels).to_csv(METRICS_DIR+'pred_labels1.csv', index = False)
        pd.DataFrame(target_hist, columns = test_labels).to_csv(METRICS_DIR+'target_labels1.csv', index = False)
...
pdtest_loss_hist.to_csv(METRICS_DIR+'test_loss1.csv',index=False)
