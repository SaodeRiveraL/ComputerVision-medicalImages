...
def train(epoch):
    ...
    pd.DataFrame(train_loss_item, columns=['epoch', 'batch', 'loss']).to_csv(
        METRICS_DIR + 'valid_loss_hist_item1_' + str(epoch) + '.csv')
...
def test(epoch):
    with torch.no_grad():
        ...
        pd.DataFrame(valid_loss_item, columns = ['epoch', 'batch','loss','acc']).to_csv(METRICS_DIR+'train_loss_hist_item1_'+str(epoch)+'.csv')
...
for epoch in range(MAX_EPOCHS):
    ...
    if valid_loss < last_valid_loss:
        ...
        filepath = 'model_finalproject.pt'
...
pdtrain_loss_hist.to_csv(METRICS_DIR+'train_loss_hist1.csv',index=False)
pdvalid_loss_hist.to_csv(METRICS_DIR+'valid_loss_hist1.csv',index=False)
